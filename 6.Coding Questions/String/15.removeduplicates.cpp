...
// TWO POINTERS:
// left  = position of the last unique element
// right = scans through the array
// If nums[right] is new, move left forward and copy nums[right] there.
// The first (left + 1) elements are the unique elements.

int main() {
  vector<int> input = {0, 0, 1, 1, 1, 2, 2, 3, 3, 4};
  cout << "Size with unique elements: " << removeDuplicates(input) << endl;
  for (int i : input) {
    cout << i << " ";
  }
  cout << endl;
